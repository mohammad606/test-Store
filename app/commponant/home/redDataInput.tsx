...
import { inputTDataype,dataStoreType} from "@/app/module/dataType";


const RedDataInput = ({data,dataStore,coc}:{data:inputTDataype,dataStore:dataStoreType,coc:string})=>{
            ...
            <div className='w-full h-auto flex border-gray-900 border-l-2'>
                <div className='mx-2 flex flex-col-reverse w-full'>
                    {data.length?data?.map((e)=>{
                        ...
                        return(
                            <div key={e.id} className='w-full bg-[#1f2937] flex flex-col my-2 py-2 rounded-2xl '>
                                {e.client?
                                ...
                                <div className='w-full flex flex-row justify-around'>
                                    <div>
                                        ...
                                        <button type="submit"
                                                onClick={()=>{HandleDeletOrder(data,dataStore,e.id,items,qtn,`Input and output data/Input`,coc,'add')}}
                                        ...
                                        <button type="button"
                                            ...
                                            <Link href={`editeOrder/${e.id}/input`}>
                                ...
                                <table>
                                    ...
                                    <tr className='flex flex-col w-1/2 '>
                                        {items?.map(f=>(
                                            <td key={items.indexOf(f)} className='mb-2 border-b-2 pl-4 smm:text-[13px]'>{f}</td>
                                        ))
                                        }
                                    </tr>
                                    <tr className='flex flex-col w-1/2'>
                                        {qtn?.map(d=> {
                                          return ( <td key={qtn.indexOf(d)} className='mb-2 border-b-2 w-full text-center smm:text-[13px]'>{d}</td>)
                                        })
                                        }
                                    </tr>
                                    </tbody>
